...
def visdom_plot(viz, win, folder, game, name, num_steps, bin_size=100, smooth=1):
    ...
    return viz.image(image, win=win), tx, ty


def process_to_tb(tx, ty): 
    ...
    if tx != None and ty != None: 
        ...
        print('last ind: {} '.format(ind))
        input('Tx: {}, Ty: {}'.format(tx[ind], ty[ind]))
        return tx[ind], ty[ind]

    else: 
        return None, None


def get_reward_log(folder, bin_size=100, smooth=1): 
    tx, ty = load_data(folder, smooth, bin_size)
    # if tx is None or ty is None:
    return tx, ty 
